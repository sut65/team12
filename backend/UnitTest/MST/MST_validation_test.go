...
func TestMST(t *testing.T) {
	...
	t.Run("Description cannot be blank", func(t *testing.T) {
		...
		ok, err := govalidator.ValidateStruct(mst)
	...
	t.Run("Date cannot be Past", func(t *testing.T) {
		mst := MST{
			Description: "asdasd",
			RegDateTime: time.Now(),
			MSTDateTime: time.Now().Add(time.Hour * -24),
		}
		ok, err := govalidator.ValidateStruct(mst)
		...
		g.Expect(err.Error()).To(gomega.Equal("Date cannot be Past"))
	})
}
