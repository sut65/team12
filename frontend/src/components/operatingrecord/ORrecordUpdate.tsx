...
function ORrecordUpdate() {
  ...
  const getORrecordID = async () => {
    let res = await GetORrecordID()
    if(res){
      setORrecord(res)
    }
  }
  ...
  const handleInputChange = (
    ...
    const id = event.target.id as keyof typeof ORrecordUpdate;
  ...
  async function submit() {
    ...
    let res = await UpdateORrecord(data);
    if (res) {
      setSuccess(true);
    } else {
      setError(true);
        ...
        >
          <Box sx={{ paddingX: 4, paddingY: 1 }}>
            <Typography
              ...
              gutterBottom
            >
              แก้ไขบันทึกการเข้าใช้งานห้องผ่าตัดที่ {orrecord?.ID}
            </Typography>
          </Box>
        </Box>
        <Divider />
        ...
        {/* <Grid item xs={6}>
          ...
          <p>ผลการผ่าตัด *</p>
            <TextField className='StyledTextField'
                ...
                inputProps={{name: "OperatingResult",}}
                value={orrecord.OperatingResult}
                />
          </Grid>
